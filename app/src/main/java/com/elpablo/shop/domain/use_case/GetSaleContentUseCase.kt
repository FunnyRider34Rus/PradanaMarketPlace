...
import com.elpablo.shop.domain.model.SaleContent
import com.elpablo.shop.domain.model.toSaleContent
...
class GetSaleContentUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Response<List<SaleContent>>> = flow {
        try {
            emit(Response.Loading())
            val content = repository.getSaleContent().map { it.toSaleContent() }
